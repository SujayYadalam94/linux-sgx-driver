 ...
 * that index.
 */
unsigned long ptr_for_index(uint8_t order, uint16_t index,
  uint16_t tree_index)
{
 ...
 * address and order.
 */
void index_for_ptr(unsigned long ptr, uint8_t order,
  uint16_t *index, uint16_t *tree_index)
{
  ...
  *index = ((ptr - tree_base_ptr) >> (LARGE_PAGE_LOG2 - order)) +
    (1 << order) - 1;
		//pr_info("%s: index=%d tree_index=%d\n", __func__, *index, *tree_index);
}
...
void sgx_free_page_buddy(struct sgx_epc_page *entry) {
  ...
  while (index != 0) {
    ...
    __list_del_entry(&epc_page->list);
    index = (index - 1) / 2;
    order--;
		//pr_info("%s2\n",__func__);
  }
	//pr_info("%s3, index, tree_index\n",__func__, index, tree_index);
...
struct sgx_epc_page *find_page_with_pa(unsigned long addr, uint8_t order)
{
